...
export class ProfilesPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private githubServiceProvider: GithubServiceProvider) {
  }
  ...
  getProfile(username){
    this.githubServiceProvider.getProfile(username).subscribe(response => {
  ...
  getRepos(username){
    this.githubServiceProvider.getRepos(username).subscribe(response =>{
      this.repos = response;
